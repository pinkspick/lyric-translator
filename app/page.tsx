...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-stone-950 text-white p-6 max-w-2xl mx-auto pb-20">
      ...
      <p className="text-stone-500 mb-4 text-sm">Simplified Chinese · Pinyin · English</p>

      <div className="flex gap-2 mb-4">
        <button
          onClick={() => setView('search')}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${view === 'search' ? 'bg-rose-500 text-white' : 'bg-stone-800 text-stone-400'}`}
        >
          Search
        </button>
        <button
          onClick={() => { setView('history'); loadHistory() }}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${view === 'history' ? 'bg-rose-500 text-white' : 'bg-stone-800 text-stone-400'}`}
        >
      ...
      {view === 'history' && (
        <div className="space-y-2">
          {history.length === 0 && (
            <p className="text-stone-500 text-sm">No saved songs yet. Search a song to save it.</p>
          )}
          {history.map(item => (
            <div key={item.key} className="flex items-center justify-between bg-stone-900 border border-stone-800 rounded-xl px-4 py-3">
              <button className="text-left flex-1" onClick={() => loadFromHistory(item)}>
                <p className="text-white font-medium">{item.title}</p>
                <p className="text-stone-400 text-sm">{item.artist}</p>
              </button>
              <button
                onClick={() => deleteFromHistory(item.key)}
                className="text-stone-600 hover:text-red-400 ml-4 text-lg"
              >
                ✕
              </button>
            </div>
